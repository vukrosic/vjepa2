"""Benchmark for fused_add_add_tensors kernel."""
...
for name, shape in SHAPES.items():
    ...
    z = torch.randn(*shape["x"], dtype=torch.float16, device="cuda")
    base_ms = bench_cuda(lambda: baseline_fn(x, y, z))
    kern_ms = bench_cuda(lambda: kernel_fn(x, y, z))
