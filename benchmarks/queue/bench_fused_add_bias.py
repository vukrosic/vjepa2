"""Benchmark for fused_add_bias kernel."""
...
for name, shape in SHAPES.items():
    ...
    bias = torch.randn(*shape["bias"], dtype=torch.float16, device="cuda")
    base_ms = bench_cuda(lambda: baseline_fn(x, bias))
    kern_ms = bench_cuda(lambda: kernel_fn(x, bias))
