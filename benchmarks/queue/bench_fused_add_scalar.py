"""Benchmark for fused_add_scalar kernel."""
import json, sys, pathlib, torch
ROOT = pathlib.Path(__file__).resolve().parents[2]
sys.path.insert(0, str(ROOT))
from src.models.utils.kernels.fused_add_scalar import kernel_fn, baseline_fn, SHAPES
...
for name, shape in SHAPES.items():
    ...
    scalar = shape.get("scalar", 1e-5)
    base_ms = bench_cuda(lambda: baseline_fn(x, scalar))
    kern_ms = bench_cuda(lambda: kernel_fn(x, scalar))
