"""Benchmark for fused_add_tanh kernel."""
import json, sys, pathlib, torch
ROOT = pathlib.Path(__file__).resolve().parents[2]
sys.path.insert(0, str(ROOT))
from src.models.utils.kernels.fused_add_tanh import kernel_fn, baseline_fn, SHAPES
...
for name_shape, shape in SHAPES.items():
    x1 = torch.randn(*shape["x1"], dtype=torch.float16, device="cuda")
    x2 = torch.randn(*shape["x2"], dtype=torch.float16, device="cuda")
    base_ms = bench_cuda(lambda: baseline_fn(x1, x2))
    kern_ms = bench_cuda(lambda: kernel_fn(x1, x2))
