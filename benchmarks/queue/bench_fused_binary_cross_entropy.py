"""Benchmark for fused binary_cross_entropy kernel."""
import json
import sys
import pathlib
...
for name, shape in SHAPES.items():
    pred = torch.rand(*shape["pred"], dtype=torch.float16, device="cuda")
    target = torch.rand(*shape["target"], dtype=torch.float16, device="cuda")
