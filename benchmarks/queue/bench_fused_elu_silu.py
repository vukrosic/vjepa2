"""Benchmark for fused_elu_silu kernel."""
import json, sys, pathlib, torch
ROOT = pathlib.Path(__file__).resolve().parents[2]
sys.path.insert(0, str(ROOT))
from src.models.utils.kernels.fused_elu_silu import kernel_fn, baseline_fn, SHAPES
...
for name, shape in SHAPES.items():
    ...
    gate = torch.randn(shape["x"], dtype=torch.float16, device="cuda")
    base_ms = bench_cuda(lambda: baseline_fn(x, gate))
    kern_ms = bench_cuda(lambda: kernel_fn(x, gate))
