"""Benchmark for fused_gelu_dropout kernel."""
import json
import sys
import pathlib
...
for name, shape in SHAPES.items():
    ...
    base_ms = bench_cuda(lambda: baseline_fn(x, drop_p=0.0, training=False))
    kern_ms = bench_cuda(lambda: kernel_fn(x, drop_p=0.0, training=False))
