"""Benchmark for fused_huber_loss kernel."""
import json, sys, pathlib, torch
ROOT = pathlib.Path(__file__).resolve().parents[2]
sys.path.insert(0, str(ROOT))
from src.models.utils.kernels.fused_huber_loss import kernel_fn, baseline_fn, SHAPES
...
for name, shape in SHAPES.items():
    ...
    base_ms = bench_cuda(lambda: baseline_fn(pred, target, shape["beta"]))
    kern_ms = bench_cuda(lambda: kernel_fn(pred, target, shape["beta"]))
