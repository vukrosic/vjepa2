...
def bench(shape_name, dtype):
    ...
    warmup = 30; iters = 200
    # Baseline
    for _ in range(warmup): torch.cuda.synchronize(); baseline_fn(a, b)
    torch.cuda.synchronize()
    t0 = torch.cuda.Event(enable_timing=True); t1 = torch.cuda.Event(enable_timing=True)
    t0.record()
    for _ in range(iters): baseline_fn(a, b)
    t1.record(); torch.cuda.synchronize()
    t_bas = t0.elapsed_time(t1) / iters
    # Kernel
    for _ in range(warmup): torch.cuda.synchronize(); kernel_fn(a, b)
    torch.cuda.synchronize()
    t0.record()
    for _ in range(iters): kernel_fn(a, b)
    t1.record(); torch.cuda.synchronize()
