"""Benchmark for fused_layernorm_add kernel."""
...
for name, shape in SHAPES.items():
    ...
    bias = torch.zeros(D, dtype=torch.float16, device="cuda")
    base_ms = bench_cuda(lambda: baseline_fn(x, residual, weight, bias))
    kern_ms = bench_cuda(lambda: kernel_fn(x, residual, weight, bias))
