"""Benchmark for fused_masked_fill_softmax kernel."""
...
for name, shape in SHAPES.items():
    ...
    scale = shape["scale"]
    base_ms = bench_cuda(lambda: baseline_fn(scores, mask, scale))
    kern_ms = bench_cuda(lambda: kernel_fn(scores, mask, scale))
