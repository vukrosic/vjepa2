"""Benchmark for fused_percentile_scale kernel."""
...
for name, shape in SHAPES.items():
    ...
    gamma = torch.randn(shape["gamma"], dtype=torch.float32, device="cuda")
    base_ms = bench_cuda(lambda: baseline_fn(x, gamma))
    kern_ms = bench_cuda(lambda: kernel_fn(x, gamma))
