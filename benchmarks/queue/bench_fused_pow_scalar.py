"""Benchmark for fused_pow_scalar kernel."""
...
for name, shape in SHAPES.items():
    x = (torch.randn(shape["x"], dtype=torch.float32, device="cuda").abs() + 0.1)
    exp = shape.get("exponent", 2.0)
    base_ms = bench_cuda(lambda: baseline_fn(x, exp))
    kern_ms = bench_cuda(lambda: kernel_fn(x, exp))
