"""Benchmark for fused_temporal_pool kernel."""
import json, sys, pathlib, torch
ROOT = pathlib.Path(__file__).resolve().parents[2]
sys.path.insert(0, str(ROOT))
from src.models.utils.kernels.fused_temporal_pool import kernel_fn, baseline_fn, SHAPES
...
for name, shape in SHAPES.items():
    ...
    base_ms = bench_cuda(lambda: baseline_fn(x, pool_size))
    kern_ms = bench_cuda(lambda: kernel_fn(x, pool_size))
