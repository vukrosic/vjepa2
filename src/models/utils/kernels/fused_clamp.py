...
@triton.jit
def _fused_clamp_fwd(X, Y, N: tl.constexpr, BLOCK: tl.constexpr,
...
@triton.jit
def _fused_clamp_bwd(X, DY, DX, N: tl.constexpr, BLOCK: tl.constexpr,
                      min_val: tl.constexpr, max_val: tl.constexpr):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < N
...
class FusedClamp(torch.autograd.Function):
    ...
    def forward(ctx, x, min_val=-1.0, max_val=1.0):
        ...
        _fused_clamp_fwd[grid](x, y, N, BLOCK=BLOCK, min_val=min_val, max_val=max_val, num_warps=4)
        return y
    ...
    def backward(ctx, dy):
        ...
        BLOCK = 1024
        grid = ((N + BLOCK - 1) // BLOCK,)
        _fused_clamp_bwd[grid](x, dy, dx, N, BLOCK=BLOCK, min_val=ctx.min_val, max_val=ctx.max_val, num_warps=4)
        return dx, None, None
