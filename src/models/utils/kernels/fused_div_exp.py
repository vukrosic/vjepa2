"""Fused Div + Exp kernel.

Pattern: y = exp(x) / other — fused exponential divided by other tensor.
"""
import torch
import triton
...
@triton.jit
def _div_exp_fwd(X, Other, Y, N: tl.constexpr, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    ...
    tl.store(Y + offs, ev / tl.where(ov == 0, 1e-12, ov), mask=mask)


@triton.jit
def _div_exp_bwd(X, Other, Y, DY, DX, DO, N: tl.constexpr, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < N
    xv = tl.load(X + offs, mask=mask).to(tl.float32)
    ...
    # d(exp/other)/dx = exp/other
    tl.store(DX + offs, dy * ev / ov_safe, mask=mask)
    # d(exp/other)/dother = -exp/other^2
    tl.store(DO + offs, -dy * ev / (ov_safe * ov_safe), mask=mask)


class FusedDivExp(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, other):
        ...
        _div_exp_fwd[grid](x, other, y, N, BLOCK=BLOCK, num_warps=4)
        return y
    ...
    def backward(ctx, dy):
        ...
        BLOCK = 1024
        grid = ((N + BLOCK - 1) // BLOCK,)
        _div_exp_bwd[grid](x, other, ctx.needs_input_grad[0], dy, dx, do, N, BLOCK=BLOCK, num_warps=4)
        return dx, do


def kernel_fn(x, other):
    return FusedDivExp.apply(x, other)
