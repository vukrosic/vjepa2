"""Fused Softplus kernel.
Pattern: y = log(exp(x) + 1)
Pure scalar loads.
"""
import torch
import triton
...
@triton.jit
def _softplus_fwd(X, Y, N: tl.constexpr, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    row_base = pid * BLOCK
...
@triton.jit
def _softplus_bwd(X, Y, DY, DX, N: tl.constexpr, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    row_base = pid * BLOCK
    for i in range(BLOCK):
        offs = row_base + i
...
class FusedSoftplus(torch.autograd.Function):
    ...
    def forward(ctx, x):
        ...
        _softplus_fwd[grid](x, y, N, BLOCK=BLOCK, num_warps=4)
        return y
    @staticmethod
    def backward(ctx, dy):
        x, = ctx.saved_tensors
        ...
        N = x.numel(); BLOCK = 1024
        grid = ((N + BLOCK - 1) // BLOCK,)
        _softplus_bwd[grid](x, x, dy, dx, N, BLOCK=BLOCK, num_warps=4)
        return dx
