"""Fused group_norm_1d kernel.

Pattern: torch.nn.functional.group_norm(x, num_groups=1)
Pure scalar loads.
"""
import torch
import triton
...
def baseline_fn(x):
    return torch.nn.functional.group_norm(x, num_groups=1)
...
class GroupNorm1D(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x):
        x = x.contiguous()
