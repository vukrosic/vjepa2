"""Fused L1-Loss kernel.

Pattern: loss = mean(|pred - target|)
Fuses: subtract + abs + mean into single-pass reduction.
Pure scalar loads.
"""
import torch
import triton
...
# --- KERNEL ---
@triton.jit
def _l1_loss_kernel(PRED, TARGET, OUT, N: tl.constexpr, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    row_base = pid * BLOCK
    local_sum = 0.0
    count = 0
    for i in range(BLOCK):
        ...
        d = p - t
        local_sum = local_sum + (d if d >= 0 else -d)
        count = count + 1
...
def kernel_fn(pred, target):
    ...
    _l1_loss_kernel[(n_blocks,)](pred, target, partial, N, BLOCK=BLOCK, num_warps=4)
