"""Fused Mul + Square kernel.

Pattern: y = x * other^2 — fused square of other and multiply.
"""
import torch
import triton
...
@triton.jit
def _mul_square_fwd(X, Other, Y, N: tl.constexpr, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < N
    xv = tl.load(X + offs, mask=mask).to(tl.float32)
    ov = tl.load(Other + offs, mask=mask).to(tl.float32)
    tl.store(Y + offs, xv * ov * ov, mask=mask)


@triton.jit
def _mul_square_bwd(X, Other, DY, DX, DO, N: tl.constexpr, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < N
    xv = tl.load(X + offs, mask=mask).to(tl.float32)
    ...
    sq = ov * ov
    # d(x*o^2)/dx = o^2
    tl.store(DX + offs, dy * sq, mask=mask)
    # d(x*o^2)/do = 2*x*o
    tl.store(DO + offs, dy * (2.0 * xv * ov), mask=mask)


class FusedMulSquare(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, other):
        ...
        _mul_square_fwd[grid](x, other, y, N, BLOCK=BLOCK, num_warps=4)
        return y
    ...
    def backward(ctx, dy):
        ...
        BLOCK = 1024
        grid = ((N + BLOCK - 1) // BLOCK,)
        _mul_square_bwd[grid](x, other, dy, dx, do, N, BLOCK=BLOCK, num_warps=4)
        return dx, do


def kernel_fn(x, other):
    return FusedMulSquare.apply(x, other)
