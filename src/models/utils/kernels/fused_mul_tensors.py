"""Fused mul_tensors kernel.

Pattern: y = x1 * x2
Fuses: multiply into one elementwise pass.
"""
import torch
import triton
...
@triton.jit
def _mul_tensors_fwd(X1, X2, Y, N: tl.constexpr, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
...
@triton.jit
def _mul_tensors_bwd(X1, X2, DY, DX1, DX2, N: tl.constexpr, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < N
    a = tl.load(X1 + offs, mask=mask, other=0.0).to(tl.float32)
    ...
    tl.store(DX1 + offs, dy * b, mask=mask)
    tl.store(DX2 + offs, dy * a, mask=mask)


class FusedMulTensors(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x1, x2):
        ...
        _mul_tensors_fwd[(n_blocks,)](x1, x2, y, N, BLOCK=BLOCK, num_warps=4)
    ...
    def backward(ctx, dy):
        ...
        BLOCK = 1024
        n_blocks = (N + BLOCK - 1) // BLOCK
        _mul_tensors_bwd[(n_blocks,)](x1, x2, dy, dx1, dx2, N, BLOCK=BLOCK, num_warps=4)
        return dx1, dx2


def kernel_fn(x1, x2):
    return FusedMulTensors.apply(x1, x2)
