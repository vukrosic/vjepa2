"""Fused Neg + Relu kernel.

Pattern: y = -x + relu(other)
Fuses: negate + relu + add into one elementwise pass.
"""
import torch
import triton
...
def baseline_fn(x, other):
    return -x + torch.nn.functional.relu(other)


# --- KERNEL ---
@triton.jit
def _fused_neg_relu_fwd(X, Other, Y, N: tl.constexpr, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
...
@triton.jit
def _fused_neg_relu_bwd(X, Other, DY, DX, DO, N: tl.constexpr, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < N
    o = tl.load(Other + offs, mask=mask).to(tl.float32)
...
class FusedNegRelu(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, other):
        ...
        _fused_neg_relu_fwd[grid](x, other, y, N, BLOCK=BLOCK, num_warps=4)
        return y
    ...
    def backward(ctx, dy):
        ...
        BLOCK = 1024
        grid = ((N + BLOCK - 1) // BLOCK,)
        _fused_neg_relu_bwd[grid](x, other, dy, dx, do, N, BLOCK=BLOCK, num_warps=4)
        return dx, do


def kernel_fn(x, other):
    return FusedNegRelu.apply(x, other)
