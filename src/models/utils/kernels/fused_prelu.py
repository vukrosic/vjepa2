...
Pattern: torch.nn.functional.prelu(x, torch.tensor(0.25, device=x.device))
Pure scalar loads.
"""
import torch
import triton
...
def baseline_fn(x):
    return torch.nn.functional.prelu(x, torch.tensor(0.25, device=x.device))
...
def _bwd(X, DY, DX, N: tl.constexpr, BLOCK: tl.constexpr):
    ...
    neg_slope = 0.25
    dx = tl.where(v > 0, dy, neg_slope * dy)
