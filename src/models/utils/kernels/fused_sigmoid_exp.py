"""Fused sigmoid_exp kernel.

Pattern: torch.nn.functional.sigmoid(x) * torch.exp(x)
Pure scalar loads.
"""
import torch
import triton
...
def baseline_fn(x):
    return torch.nn.functional.sigmoid(x) * torch.exp(x)
...
def _bwd(X, DY, DX, N: tl.constexpr, BLOCK: tl.constexpr):
    ...
    dsig = sig * (1.0 - sig)
    ex = tl.exp(tl.minimum(v, 40.0))
    dx = dy * (dsig * ex + sig * ex)
    tl.store(DX + offs, dx, mask=mask)


class SigmoidExp(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x):
        x = x.contiguous()
        ...
        ctx.save_for_backward(sig, ex, v)
        return y
