...
# --- KERNEL ---
@triton.jit
def _fused_softplus_fwd(X, Y, N: tl.constexpr, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < N
    x = tl.load(X + offs, mask=mask, other=0.0).to(tl.float32)
    sp = tl.log(tl.exp(tl.minimum(x, 20.0)) + 1.0)
    tl.store(Y + offs, sp, mask=mask)


@triton.jit
def _fused_softplus_bwd(X, DY, DX, N: tl.constexpr, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < N
    x = tl.load(X + offs, mask=mask, other=0.0).to(tl.float32)
...
class FusedSoftplus(torch.autograd.Function):
    ...
    def forward(ctx, x):
        ...
        _fused_softplus_fwd[(n_blocks,)](x_c, y, N, BLOCK=BLOCK, num_warps=4)
    ...
    def backward(ctx, dy):
        ...
        BLOCK = 1024
        n_blocks = (N + BLOCK - 1) // BLOCK
        _fused_softplus_bwd[(n_blocks,)](x_c, dy_c, dx, N, BLOCK=BLOCK, num_warps=4)
        return dx


def kernel_fn(x):
    return FusedSoftplus.apply(x)
