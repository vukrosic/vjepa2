"""Fused Sub + Exp kernel.

Pattern: y = exp(x - other) — fused subtract and exponential.
"""
import torch
import triton
...
@triton.jit
def _sub_exp_fwd(X, Other, Y, N: tl.constexpr, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    ...
    d = xv - ov
    yv = tl.exp(tl.minimum(d, 40.0))
    tl.store(Y + offs, yv, mask=mask)


@triton.jit
def _sub_exp_bwd(X, Other, Y, DY, DX, DO, N: tl.constexpr, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < N
    xv = tl.load(X + offs, mask=mask).to(tl.float32)
    ...
    d = xv - ov
    yv = tl.exp(tl.minimum(d, 40.0))
    tl.store(DX + offs, dy * yv, mask=mask)
    tl.store(DO + offs, -dy * yv, mask=mask)


class FusedSubExp(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, other):
        ...
        _sub_exp_fwd[grid](x, other, y, N, BLOCK=BLOCK, num_warps=4)
        return y
    ...
    def backward(ctx, dy):
        ...
        BLOCK = 1024
        grid = ((N + BLOCK - 1) // BLOCK,)
        _sub_exp_bwd[grid](x, other, ctx.needs_input_grad[0], dy, dx, do, N, BLOCK=BLOCK, num_warps=4)
        return dx, do


def kernel_fn(x, other):
    return FusedSubExp.apply(x, other)
