"""Fused Sub + Tanh kernel.

Pattern: y = tanh(x - other) — fused subtract and tanh activation.
"""
import torch
import triton
...
@triton.jit
def _sub_tanh_fwd(X, Other, Y, N: tl.constexpr, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    ...
    d = xv - ov
    e2d = tl.exp(tl.minimum(2.0 * d, 40.0))
    yv = (e2d - 1.0) / (e2d + 1.0)
    tl.store(Y + offs, yv, mask=mask)


@triton.jit
def _sub_tanh_bwd(X, Other, DY, DX, DO, N: tl.constexpr, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < N
    xv = tl.load(X + offs, mask=mask).to(tl.float32)
    ...
    d = xv - ov
    e2d = tl.exp(tl.minimum(2.0 * d, 40.0))
    tanh_v = (e2d - 1.0) / (e2d + 1.0)
    dtanh = 1.0 - tanh_v * tanh_v
    tl.store(DX + offs, dy * dtanh, mask=mask)
    tl.store(DO + offs, -dy * dtanh, mask=mask)


class FusedSubTanh(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, other):
        ...
        _sub_tanh_fwd[grid](x, other, y, N, BLOCK=BLOCK, num_warps=4)
        return y
    ...
    def backward(ctx, dy):
        ...
        BLOCK = 1024
        grid = ((N + BLOCK - 1) // BLOCK,)
        _sub_tanh_bwd[grid](x, other, dy, dx, do, N, BLOCK=BLOCK, num_warps=4)
        return dx, do


def kernel_fn(x, other):
    return FusedSubTanh.apply(x, other)
