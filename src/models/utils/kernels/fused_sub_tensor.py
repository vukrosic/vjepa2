"""Fused Subtract Tensor kernel.

Pattern: y = x - other
Fuses: elementwise subtract into one pass.
"""
import torch
import triton
...
# --- KERNEL ---
@triton.jit
def _fused_sub_tensor_fwd(X, Other, Y, N: tl.constexpr, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
...
@triton.jit
def _fused_sub_tensor_bwd(X, Other, DY, DX, DO, N: tl.constexpr, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < N
    dy = tl.load(DY + offs, mask=mask).to(tl.float32)
    tl.store(DX + offs, dy, mask=mask)
    tl.store(DO + offs, -dy, mask=mask)


class FusedSubTensor(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, other):
        ...
        _fused_sub_tensor_fwd[grid](x, other, y, N, BLOCK=BLOCK, num_warps=4)
        return y
    ...
    def backward(ctx, dy):
        ...
        BLOCK = 1024
        grid = ((N + BLOCK - 1) // BLOCK,)
        _fused_sub_tensor_bwd[grid](x, other, dy, dx, do, N, BLOCK=BLOCK, num_warps=4)
        return dx, do


def kernel_fn(x, other):
    return FusedSubTensor.apply(x, other)
