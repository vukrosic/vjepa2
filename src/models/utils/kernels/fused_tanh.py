...
Pattern: y = tanh(x) = (exp(2x) - 1) / (exp(2x) + 1)
...
@triton.jit
def _fused_tanh_fwd(X, Y, N: tl.constexpr, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
...
@triton.jit
def _fused_tanh_bwd(X, DY, DX, N: tl.constexpr, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < N
...
class FusedTanh(torch.autograd.Function):
    ...
    def forward(ctx, x):
        ...
        _fused_tanh_fwd[grid](x, y, N, BLOCK=BLOCK, num_warps=4)
        return y
    ...
    def backward(ctx, dy):
        ...
        BLOCK = 1024
        grid = ((N + BLOCK - 1) // BLOCK,)
        _fused_tanh_bwd[grid](x, dy, dx, N, BLOCK=BLOCK, num_warps=4)
        return dx
