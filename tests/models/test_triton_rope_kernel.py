...
from src.models.utils.triton_kernels import TRITON_AVAILABLE, triton_rotate_queries_or_keys_autograd
...
def test_triton_rope_rotate_matches_pytorch_forward():
    ...
    with torch.no_grad():
        reference = rotate_queries_or_keys(x, pos)
        actual = triton_rotate_queries_or_keys_autograd(x, pos, omega)

    torch.testing.assert_close(actual, reference, atol=3e-3, rtol=3e-3)


@pytest.mark.skipif(not torch.cuda.is_available() or not TRITON_AVAILABLE, reason="This test requires CUDA + Triton")
def test_triton_rope_rotate_matches_pytorch_backward():
    torch.manual_seed(0)
