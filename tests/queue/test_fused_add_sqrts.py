"""Parity test for fused_add_sqrts kernel."""
import torch, pytest
...
def test_forward_parity(shape_name, dtype):
    ...
    x = torch.randn(*shape["x"], dtype=dtype, device="cuda").abs_() + 0.1
...
def test_backward_parity(shape_name):
    if not torch.cuda.is_available():
        pytest.skip("CUDA not available")
    shape = SHAPES[shape_name]
    x1 = (torch.randn(*shape["x"], dtype=torch.float32, device="cuda").abs_() + 0.1).requires_grad_(True)
