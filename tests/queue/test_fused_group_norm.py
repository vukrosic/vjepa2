"""Parity test for fused group_norm kernel."""
import torch
import pytest
import sys, pathlib
sys.path.insert(0, str(pathlib.Path(__file__).resolve().parents[2]))
from src.models.utils.kernels.fused_group_norm import kernel_fn, baseline_fn, SHAPES
...
def test_forward_parity(shape_name, dtype):
    ...
    expected = baseline_fn(x, weight, bias, num_groups=shape["num_groups"])
    actual = kernel_fn(x, weight, bias, num_groups=shape["num_groups"])
