"""Parity test for fused_hard_sigmoid kernel."""
import torch, pytest
from src.models.utils.kernels.fused_hard_sigmoid import kernel_fn, baseline_fn, SHAPES
...
def test_backward_parity(shape_name, dtype):
    if not torch.cuda.is_available(): pytest.skip("CUDA not available")
    s = SHAPES[shape_name]
    x = torch.randn(s["x"], dtype=dtype, device="cuda", requires_grad=True)
    x_ker = x.detach().clone().requires_grad_(True)
    y_ker = kernel_fn(x_ker)
    y_bas = baseline_fn(x)
    y_ker.sum().backward(); y_bas.sum().backward()
    atol = ATOL_FP16 if dtype == torch.float16 else ATOL_FP32
