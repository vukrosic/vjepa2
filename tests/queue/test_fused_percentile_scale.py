"""Parity test for fused_percentile_scale kernel."""
import torch, pytest
...
def test_forward_parity(shape_name, dtype):
    ...
    expected = baseline_fn(x, gamma)
    actual = kernel_fn(x, gamma)
...
def test_backward_parity(shape_name, dtype):
    ...
    gamma = torch.randn(shape["gamma"], dtype=dtype, device="cuda", requires_grad=True)
    x_ker = x.detach().clone().requires_grad_(True)
    gamma_ker = gamma.detach().clone().requires_grad_(True)
    y_ker = kernel_fn(x_ker, gamma_ker)
    y_bas = baseline_fn(x, gamma)
    y_ker.sum().backward()
    y_bas.sum().backward()
    atol = ATOL_FP16 if dtype == torch.float16 else ATOL_FP32
