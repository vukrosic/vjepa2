"""Parity test for fused_rmsnorm_residual kernel."""
import torch
import pytest
from src.models.utils.kernels.fused_rmsnorm_residual import kernel_fn, baseline_fn, SHAPES
...
def test_backward_parity(shape_name):
    ...
    out1 = baseline_fn(x, residual, weight)
    out2 = kernel_fn(x_ker, res_ker, weight)
    grad = torch.randn_like(out1)
    out1.backward(grad)
    out2.backward(grad)
    torch.testing.assert_close(x_ker.grad, x.grad, atol=1e-3, rtol=1e-3)
    torch.testing.assert_close(res_ker.grad, residual.grad, atol=1e-3, rtol=1e-3)
