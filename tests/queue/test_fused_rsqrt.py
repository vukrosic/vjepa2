"""Parity test for fused_rsqrt kernel."""
import torch, pytest
from src.models.utils.kernels.fused_rsqrt import kernel_fn, baseline_fn, SHAPES
...
def test_backward_parity(shape_name):
    if not torch.cuda.is_available():
        pytest.skip("CUDA not available")
    shape = SHAPES[shape_name]
    x1 = torch.randn(*shape["x"], dtype=torch.float32, device="cuda", requires_grad=True).abs() + 0.1
