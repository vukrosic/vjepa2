"""Parity test for fused Sigmoid * Multiply kernel."""
import torch
import pytest
import sys, pathlib
sys.path.insert(0, str(pathlib.Path(__file__).resolve().parents[2]))
from src.models.utils.kernels.fused_sigmoid_mul import kernel_fn, baseline_fn, SHAPES
...
def test_forward_parity(shape_name, dtype):
    ...
    x_gate = torch.randn(*shape["x"], dtype=dtype, device="cuda")
    y_value = torch.randn(*shape["y"], dtype=dtype, device="cuda")
    expected = baseline_fn(x_gate, y_value)
    actual = kernel_fn(x_gate, y_value)
...
def test_backward_parity(shape_name):
    ...
    out_base = baseline_fn(xa, ya)
    out_kern = kernel_fn(xb, yb)
