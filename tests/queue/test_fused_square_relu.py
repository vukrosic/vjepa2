"""Parity test for fused_square_relu kernel."""
import torch, pytest
from src.models.utils.kernels.fused_square_relu import kernel_fn, baseline_fn, SHAPES
...
def test_forward_parity(shape_name, dtype):
    ...
    other = torch.randn(*shape["other"], dtype=dtype, device="cuda")
...
def test_backward_parity(shape_name, dtype):
    ...
    other1 = torch.randn(*shape["other"], dtype=dtype, device="cuda", requires_grad=True)
    ...
    torch.testing.assert_close(other2.grad, other1.grad, atol=atol, rtol=0)
