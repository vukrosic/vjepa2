...
from src.models.predictor import VisionTransformerPredictor, _get_sorted_target_positions
...
def test_src_predictor_rope_non_square_runs_cpu():
    ...
    B = 2
    enc_mask_indices = [torch.tensor([[0, 1, 2], [3, 4, 5]], dtype=torch.int64)]
    target_mask_indices = [torch.tensor([[6, 7, 0], [1, 2, 3]], dtype=torch.int64)]
    enc = torch.randn(B, enc_mask_indices[0].size(1), 96)

    with torch.no_grad():
        out = predictor(enc, enc_mask_indices, target_mask_indices)
